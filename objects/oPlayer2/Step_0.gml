...
//positive feedback
if global.previous_winner == 2 && !oHUD.flag{
	has_shield = true
	shield_timer = 500
	global.previous_winner = 0
}
...
if (gamepad_button_check_pressed(pad_id, gp_shoulderr) && cd <= 0) {
	...
	var bullet_x = x + lengthdir_x(barrel_length, oPointer2.image_angle)
	var bullet_y = y + lengthdir_y(barrel_length, oPointer2.image_angle)


	if has_shotgun && shot_count >= 0 {
		with (instance_create_layer(bullet_x, bullet_y, "bullets", oBullet)) {
			direction = oPointer2.image_angle + 5
			speed = 5
			num_bounce = 2
			if oPlayer2.rage_mode {
				damage = 20
			}
		}
		...
		with (instance_create_layer(bullet_x, bullet_y, "bullets", oBullet)) {
			direction = oPointer2.image_angle + 3
			speed = 5
			num_bounce = 2
			if oPlayer2.rage_mode {
				damage = 20
		...
		with (instance_create_layer(bullet_x, bullet_y, "bullets", oBullet)) {
			direction = oPointer2.image_angle
			speed = 20
			num_bounce = 8
			damage = 40
		}
...
if has_shield && shield_timer >= 0 {
	oShield2.is_active = true
	shield_timer --
} else {
	has_shield = false
	shield_timer = 300
	oShield2.is_active = false
}
